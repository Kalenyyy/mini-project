  ...
  const handleConfirm = async () => {
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/customers/update-status/${customerId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        toast.success("Customer berhasil diubah menjadi tidak aktif!"); // Success toast
        onSuccess?.(); // reload data or show toast
        onClose(); // close modal
      } else {
        toast.error("Gagal memperbarui status customer."); // Error toast
      }
    } catch (err) {
