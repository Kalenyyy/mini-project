...
import fetchOrderDetails from "@/app/lib/order/fetchOrderDetail";
import { formatRupiah, parseRupiah } from "../../../lib/formatRupiah";
import Breadcrumb from "../../../lib/breadcrumb/useBreadcrumb";
import {
  handleProcessOrder,
  handleCompletePayment,
} from "../../../lib/order/helperUpdateOrder";

export default function OrderEditClient({ id }) {
  ...
  useEffect(() => {
    if (id) {
      ...
      fetchOrderDetails(id, setOrder, setSelectedCustomer, setSelectedProducts);
      setIsClient(true);
    }
  ...
  const productOptions = products.map((p) => ({
    value: p.id, // The ID is the 'value' for the Select
    label: `${p.name} (${formatRupiah(p.price)})`, // The label is the name and formatted price
    price: p.price, // Keep the price for calculation purposes
  }));

  const handleAddProduct = () => {
    const newProduct = {
      product: { id: "", name: "", price: 0 }, // Produk kosong awal
      quantity: 1,
      subtotal: 0,
    };
  ...
  const handleProductChange = (index, selectedOption) => {
    console.log("Selected Option:", selectedOption); // Log pilihan produk yang dipilih
    const updated = [...selectedProducts];

    if (selectedOption) {
      // Update produk dengan data baru
      updated[index].product = {
        id: selectedOption.value, // ID produk
        name: selectedOption.label.split(" (")[0], // Nama produk
        price: selectedOption.price, // Harga produk
      };
      updated[index].subtotal =
        updated[index].quantity * updated[index].product.price;

      console.log("Updated Product at index", index, ":", updated[index]); // Log produk yang diperbarui
    }

    console.log("Updated Products Array:", updated); // Log array produk setelah diupdate
    setSelectedProducts(updated); // Update state dengan produk yang sudah diubah
  };

  const handleQuantityChange = (index, quantity) => {
    // Menambahkan validasi pada quantity, memastikan jumlahnya selalu lebih dari atau sama dengan 1
    const updatedQuantity = Math.max(1, Number(quantity) || 1);

    // Membuat salinan dari selectedProducts untuk menghindari mutasi langsung
    const updated = [...selectedProducts];

    // Mengambil harga produk yang sesuai
    const productPrice = updated[index].product?.price || 0;

    // Memperbarui quantity dan subtotal produk pada index yang sesuai
    updated[index].quantity = updatedQuantity;
    updated[index].subtotal = updatedQuantity * productPrice;

    // Update state selectedProducts dengan data yang baru
    setSelectedProducts(updated);
  };
  ...
  // Menghitung total_amount yang sudah diformat
  const numericPaymentAmount = paymentAmount ? parseRupiah(paymentAmount) : 0;
  const numericTotalAmount = selectedProducts.reduce((acc, item) => {
    const price = item.product?.price || 0;
    const quantity = item.quantity || 0;
    return acc + price * quantity;
  }, 0);

  // Menghitung selisih antara pembayaran dan total amount
  const change = numericPaymentAmount - numericTotalAmount;
  // Cek apakah pembayaran cukup
  const isPaymentValid = numericPaymentAmount >= numericTotalAmount;

  const breadcrumbItems = [
    { label: "Home", href: "/" },
    { label: "Order List", href: "/orders" },
    { label: "Edit Order #" + id, href: "#" }, // Tidak ada href = halaman aktif
  ];
    ...
    <Layout>
      ...
      <div className=" mx-auto p-6 space-y-8 bg-white">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-gray-800">Edit Order #{id}</h1>
          <p className="text-gray-600">
            Edit detail customer dan produk yang dipesan
          </p>
        </div>
          ...
          <div>
            ...
            {isClient && (
              <Select
                ...
                value={
                  selectedCustomer || { value: "", label: "Pilih customer" }
                } // Default value jika kosong
                onChange={(newValue) => {
                  setSelectedCustomer(newValue || null); // Memastikan tidak null
                }}
                isClearable
                isSearchable
                isLoading={loading}
                ...
                {selectedProducts.map((item, index) => (
                  ...
                  >
                    <div className="w-full md:w-2/5">
                      {isClient && (
                        <Select
                          options={productOptions} // Menyediakan daftar produk
                          value={{
                            value: item.product?.id, // ID produk yang dipilih
                            label: item.product?.name
                              ? `${item.product?.name} (${formatRupiah(
                                  item.product?.price
                                )})`
                              : "Pilih produk...", // Menampilkan nama produk dan harga
                          }}
                          onChange={(selectedOption) => {
                            console.log("Product selected:", selectedOption); // Log untuk melihat apa yang dipilih
                            handleProductChange(index, selectedOption); // Panggil handleProductChange saat memilih produk
                          }}
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {formatRupiah(numericTotalAmount)}
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="flex justify-end">
              <div className="relative group">
                <button
                  onClick={() =>
                    handleCompletePayment(
                      order, // You would pass your order object here
                      selectedProducts,
                      selectedCustomer,
                      isPaymentValid,
                      total,
                  ...
                  <div className="absolute z-10 invisible group-hover:visible bottom-full mb-2 w-64 px-3 py-2 text-sm font-medium text-white bg-gray-700 rounded-lg shadow-sm">
                    {!paymentAmount
                      ...
                      : `Jumlah pembayaran kurang ${formatRupiah(
                          numericTotalAmount - numericPaymentAmount
                        )}`}
                  </div>
                )}
              </div>
